    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See atlaz_state._WAL_INCOMPAT_MARKERS for detection logic.
                from atlaz_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from atlaz_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from atlaz_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from atlaz_cli.profiles import profile_exists as _pe
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``atlaz_cli.main`` is the console-script target declared in
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m atlaz_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``atlaz_cli`` sits below
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* atlaz_constants is imported.
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``atlaz_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from atlaz_constants import get_default_hermes_root
