...
Usage:
    python -m atlaz_cli.main web          # Start on http://127.0.0.1:9119
    python -m atlaz_cli.main web --port 8080
"""
...
#
# This list is defined in ``atlaz_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from atlaz_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from atlaz_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    try:
        from atlaz_cli.dashboard_auth import list_providers as _list_providers
...
_ACTION_LOG_DIR: Path = get_atlaz_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``atlaz_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "atlaz_cli.main", *subcommand]
...
# in atlaz_cli/config.py — listed here for deterministic ordering in the UI.
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from atlaz_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from atlaz_cli.auth import clear_provider_auth
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from atlaz_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from atlaz_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_atlaz_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from atlaz_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from atlaz_cli import profiles as profiles_mod
    try:
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_atlaz_home()
...
async def get_skills():
    ...
    from atlaz_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from atlaz_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from atlaz_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from atlaz_cli.dashboard_auth.audit import AuditEvent, audit_log
        from atlaz_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    matching what ``atlaz_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from atlaz_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from atlaz_cli.dashboard_auth.ws_tickets import mint_ticket
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`atlaz_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from atlaz_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_atlaz_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as atlaz_cli.plugins):
    ...
    from atlaz_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_atlaz_home() / "plugins", "user"),
    ...
    # ``atlaz_cli/plugins.py`` and the documented user contract.
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from atlaz_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_atlaz_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from atlaz_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from atlaz_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from atlaz_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from atlaz_cli.plugins_cmd import dashboard_remove_user_plugin
...
from atlaz_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from atlaz_cli.dashboard_auth import list_providers
