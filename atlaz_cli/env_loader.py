...
# several hot modules (cli.py, atlaz_cli/main.py, run_agent.py,
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``atlaz_cli.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from atlaz_cli.config import _sanitize_env_lines
    except ImportError:
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    time from several hot modules (cli.py, atlaz_cli/main.py,
