...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "atlaz_cli.main"]
    if profile != "default":
...
def _gateway_list() -> None:
    ...
    """
    try:
        from atlaz_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
User={username}
Group={group_name}
ExecStart={python_path} -m atlaz_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def _setup_whatsapp():
    ...
    from atlaz_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from atlaz_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``atlaz_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from atlaz_cli import setup as _s
    return {
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`atlaz_cli.service_manager` are caught and surfaced as
