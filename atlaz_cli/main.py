...
def _print_fast_version_info() -> None:
    from atlaz_cli import __release_date__, __version__
...
def _apply_profile_override() -> None:
    ...
    # Mirrors atlaz_cli.profiles._PROFILE_ID_RE so we never call
    ...
    if profile_name is not None:
        try:
            from atlaz_cli.profiles import resolve_profile_env
...
from atlaz_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from atlaz_cli.config import get_env_path, get_hermes_home, load_config
    from atlaz_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from atlaz_cli.config import DEFAULT_CONFIG
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from atlaz_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from atlaz_cli.oneshot import _normalize_toolsets
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from atlaz_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from atlaz_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from atlaz_cli.config import load_config as _load_config_for_xai_check
...
def cmd_gateway(args):
    ...
    from atlaz_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from atlaz_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from atlaz_cli.config import get_env_value, save_env_value
...
def cmd_setup(args):
    ...
    from atlaz_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from atlaz_cli.config import stamp_install_method
    from atlaz_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from atlaz_cli.providers import resolve_provider_full
    ...
    from atlaz_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _clear_stale_openai_base_url():
    ...
    """
    from atlaz_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`atlaz_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from atlaz_cli.config import load_config
    from atlaz_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from atlaz_cli.auth import _prompt_model_selection
    from atlaz_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from atlaz_cli.setup import _curses_prompt_choice
...
>>>>>>> origin/main:hermes_cli/main.py
    ...
    from atlaz_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from atlaz_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from atlaz_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from atlaz_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from atlaz_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from atlaz_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from atlaz_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from atlaz_cli.auth import _save_model_choice, deactivate_provider
    from atlaz_cli.config import get_env_value, load_config, save_config
    from atlaz_cli.secret_prompt import masked_secret_prompt
    ...
    from atlaz_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from atlaz_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from atlaz_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from atlaz_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from atlaz_cli import azure_detect
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from atlaz_cli.curses_ui import flush_stdin
...
# downstream call sites read `atlaz_cli.main._PROVIDER_MODELS` directly,
...
def _model_flow_copilot(config, current_model=""):
    ...
    from atlaz_cli.config import save_env_value, load_config, save_config
    from atlaz_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from atlaz_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from atlaz_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from atlaz_cli.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from atlaz_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from atlaz_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from atlaz_cli.auth import AuthError
        from atlaz_cli.models import fetch_lmstudio_models
        ...
        from atlaz_cli.models import fetch_ollama_cloud_models
...
def cmd_login(args):
    ...
    from atlaz_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from atlaz_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from atlaz_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from atlaz_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from atlaz_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from atlaz_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from atlaz_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from atlaz_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from atlaz_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from atlaz_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from atlaz_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from atlaz_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from atlaz_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from atlaz_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from atlaz_cli.backup import run_quick_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from atlaz_cli.banner import check_for_updates
        from atlaz_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from atlaz_cli.uninstall import run_uninstall
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``atlaz_cli/web_dist/`` (per vite.config.ts
    outDir: "../atlaz_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "atlaz_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``atlaz_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m atlaz_cli.main web` invocation.
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        "hermes dashboard",
        "atlaz_cli.main dashboard",
        "atlaz_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `atlaz_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://atlaz.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/Axighi/atlaz/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/Axighi/atlaz.git",
    "git@github.com:NousResearch/hermes-agent.git",
    "https://github.com/Axighi/atlaz",
    "git@github.com:NousResearch/hermes-agent",
}
OFFICIAL_REPO_URL = "https://github.com/Axighi/atlaz.git"
    ...
    if not has_upstream:
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Axighi/atlaz.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Axighi/atlaz.git' to add later."
            )
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from atlaz_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # atlaz_cli.config.get_hermes_home to simulate setup failure.
        from atlaz_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from atlaz_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from atlaz_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from atlaz_cli.config import recommended_update_command
        from atlaz_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from atlaz_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
...
def _cmd_update_pip(args):
    ...
    from atlaz_cli import __version__
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/Axighi/atlaz/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        try:
            from atlaz_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in atlaz_cli/config.py bricked
        ...
        try:
            from atlaz_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from atlaz_cli.tools_config import install_cua_driver
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from atlaz_cli import profiles as _profiles_mod
        ...
        from atlaz_cli import profile_describer as _pd
        ...
        from atlaz_cli.profiles import profile_exists
        ...
        from atlaz_cli.profiles import rename_profile
        ...
        from atlaz_cli.profiles import export_profile
        ...
        from atlaz_cli.profiles import import_profile
        ...
        import tempfile
        from atlaz_cli.profile_distribution import (
            plan_install,
        ...
        from atlaz_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from atlaz_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from atlaz_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "atlaz_cli" / "web_dist"
        )
    ...
    from atlaz_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from atlaz_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from atlaz_cli.logs import tail_log, list_logs
...
# the top-level flags declared in ``atlaz_cli/_parser.py``.
#
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from atlaz_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from atlaz_cli.oneshot import run_oneshot
...
def main():
    ...
    # =========================================================================
    from atlaz_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://atlaz.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://atlaz.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from atlaz_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from atlaz_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from atlaz_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from atlaz_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from atlaz_cli.kanban import build_parser as _build_kanban_parser
...
""",
    ...
    from atlaz_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_pairing(args):
        from atlaz_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from atlaz_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from atlaz_cli.skills_hub import skills_command
    ...
    from atlaz_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from atlaz_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from atlaz_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from atlaz_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from atlaz_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from atlaz_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from atlaz_cli.tools_config import tools_command
    ...
    def cmd_mcp(args):
        from atlaz_cli.mcp_config import mcp_command
    ...
    def cmd_claw(args):
        from atlaz_cli.claw import claw_command
...
""",
    ...
    from atlaz_cli.config import get_container_exec_info
