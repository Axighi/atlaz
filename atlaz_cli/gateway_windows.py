...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m atlaz_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    ...
    from atlaz_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "atlaz_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during atlaz_cli boot.
    from atlaz_cli.gateway import _profile_suffix
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m atlaz_cli.main [--profile X] gateway run``
    ...
    # if someone imports atlaz_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "atlaz_cli.main"]
    if profile_arg:
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "atlaz_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m atlaz_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    from atlaz_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from atlaz_cli.gateway import find_gateway_pids, _profile_arg
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from atlaz_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
