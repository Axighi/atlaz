...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains atlaz_cli reference)
                content = wrapper.read_text()
                if 'atlaz_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from atlaz_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from atlaz_cli.gateway import get_launchd_plist_path
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m atlaz_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "atlaz_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Axighi/atlaz/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Axighi/atlaz/main/scripts/install.sh | bash", Colors.DIM))
        print()
