...
- ``atlaz_cli.providers``        -- canonical provider identity + overlays
- ``atlaz_cli.model_normalize``  -- per-provider name formatting
...
from atlaz_cli.providers import (
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from atlaz_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                from atlaz_cli.config import validate_config_structure
    ...
    # Mirrors the same logic in atlaz_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from atlaz_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from atlaz_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://atlaz.nousresearch.com/docs/api/model-catalog.json so
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from atlaz_cli.auth import _load_auth_store
    ...
    from atlaz_cli.providers import HERMES_OVERLAYS
    from atlaz_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    # with `hermes model`).
    try:
        from atlaz_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see atlaz_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, atlaz_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`atlaz_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from atlaz_cli.models import fetch_openrouter_models
