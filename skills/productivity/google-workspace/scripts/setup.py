...
# Ensure sibling modules (_atlaz_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _atlaz_home import display_atlaz_home, get_atlaz_home

HERMES_HOME = get_atlaz_home()
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_atlaz_home()}/google_token.json")
