...
- **Agent process.** The Python interpreter running ATLAZ,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted ATLAZ access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in ATLAZ's documentation
...
ATLAZ supports two OS-level isolation postures. They address
...
Under a whole-process wrapper, ATLAZ's in-process heuristics
...
ATLAZ filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in ATLAZ
itself. Bugs in ATLAZ's plugin-install or plugin-discovery
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, ATLAZ's own documentation, or reasonable
  operator expectations would predict — including cases where
  ATLAZ has documented a stance about how its output should
...
- ATLAZ includes supply-chain guards for MCP server
