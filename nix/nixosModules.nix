# nix/nixosModules.nix — NixOS module for atlaz
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    ...
    # Deep-merge config type (from 0xrsydn/nix-atlaz)
    ...
    options.services.atlaz = with lib; {
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in atlaz's
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.atlaz" ]; then
                  rm -f "${symlinkPath}"
                  echo "atlaz: removed symlink ${symlinkPath}"
                fi
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "atlaz: backing up existing ${symlinkPath} to $_backup"
