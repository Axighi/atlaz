...
| **GitHub 事件** | PR 开启、推送、issue、CI 结果时触发 | Webhook 平台（`atlaz webhook subscribe`） |
| **API 调用** | 外部服务向你的端点 POST JSON | Webhook 平台（config.yaml 路由或 `atlaz webhook subscribe`） |
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (atlaz_cli/commands.py, atlaz_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (atlaz_cli/config.py) — may need docs/user-guide/configuration.md update
