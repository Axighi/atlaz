...
| **额外技能来源**（自定义 GitHub 仓库、私有技能索引） | [技能](/user-guide/features/skills)——`atlaz skills tap add <repo>` · [发布 tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
- `ctx.register_cli_command()` 注册 CLI 子命令（例如 `atlaz my-plugin <subcommand>`）
...
- **未在配置中启用**——插件需要手动启用。运行 `atlaz plugins enable <name>`（名称来自 `plugins list` 输出，嵌套布局下可能是 `<category>/<plugin>`）。
...
用户运行 `atlaz plugins install` 时，会**交互式提示**输入任何缺失的 `requires_env` 变量。值会自动保存到 `.env`。
...
注册后，用户可以运行 `atlaz my-plugin status`、`atlaz my-plugin config` 等命令。
...
|---|---|---|
| 调用方式 | 会话中的 `/name` | 终端中的 `atlaz name` |
...
def register(ctx):
    ctx.register_platform(
        ...
        # 在 `atlaz gateway status` 中显示，无需 SDK 实例化。
