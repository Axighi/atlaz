...
sidebar_position: 14
title: "AWS Bedrock"
description: "将 ATLAZ 与 Amazon Bedrock 配合使用——原生 Converse API、IAM 身份验证、Guardrails 及跨区域推理"
---

# AWS Bedrock

ATLAZ 通过 **Converse API** 原生支持 Amazon Bedrock——而非 OpenAI 兼容端点。这让你可以完整访问 Bedrock 生态系统：IAM 身份验证、Guardrails、跨区域推理配置文件以及所有基础模型。
...
运行 `atlaz model` 后，你的 `~/.hermes/config.yaml` 将包含以下内容：
...
Bedrock 模型使用**推理配置文件 ID** 进行按需调用。`atlaz model` 选择器会自动显示这些 ID，并将推荐模型置于顶部：
