...
本指南介绍如何将 ATLAZ 连接到 GitHub，使其自动获取 pull request 的 diff、分析代码变更并发布评论——由 webhook 事件触发，无需手动 prompt（提示词）。
...
:::note
`atlaz webhook test <name>` 仅适用于通过 `atlaz webhook subscribe` 创建的**动态订阅**。它不读取 `config.yaml` 中的路由。
:::
