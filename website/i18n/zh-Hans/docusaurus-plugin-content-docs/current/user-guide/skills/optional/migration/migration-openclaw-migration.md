---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 ATLAZ"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 ATLAZ"
---
...
将用户的 OpenClaw 自定义配置迁移到 ATLAZ。从 `~/.openclaw` 导入 Hermes 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `atlaz skills install official/migration/openclaw-migration` 安装 |
| 路径 | `optional-skills/migration/openclaw-migration` |
| 版本 | `1.0.0` |
| 作者 | ATLAZ (Nous Research) |
| 许可证 | MIT |
...
当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 ATLAZ 时，使用此 skill。
...
**首次设置：** `atlaz setup` 向导会自动检测 `~/.openclaw`，并在配置开始前提供迁移选项。
