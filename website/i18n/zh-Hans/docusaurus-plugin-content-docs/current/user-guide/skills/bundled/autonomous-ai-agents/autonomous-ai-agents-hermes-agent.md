---
title: "ATLAZ — 配置、扩展或贡献 ATLAZ"
sidebar_label: "ATLAZ"
description: "配置、扩展或贡献 ATLAZ"
---
...
ATLAZ 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Hermes 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。
...
**此 skill 帮助你高效使用 ATLAZ** — 包括设置、配置功能、生成额外的 agent 实例、排查问题、找到正确的命令和设置，以及在需要扩展或贡献时理解系统的工作原理。

**文档：** https://atlaz.nousresearch.com/docs/
...
平台文档：https://atlaz.nousresearch.com/docs/user-guide/messaging/
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://atlaz.nousresearch.com/docs/reference/slash-commands)。命令注册表的权威来源是 `atlaz_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
使用 `atlaz config edit` 或 `atlaz config set section.key value` 编辑。
...
完整配置参考：https://atlaz.nousresearch.com/docs/user-guide/configuration

### 提供商

支持 20+ 个提供商。通过 `atlaz model` 或 `atlaz setup` 设置。
...
完整提供商文档：https://atlaz.nousresearch.com/docs/integrations/providers

### Toolset

通过 `atlaz tools`（交互式）或 `atlaz tools enable/disable NAME` 启用/禁用。
...
**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export HERMES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `atlaz config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
要完全阻止模型访问网络或媒体工具，打开 `atlaz tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
- **fire-and-forget 使用 `atlaz chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`atlaz cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用户文档：https://atlaz.nousresearch.com/docs/user-guide/features/cron

### Curator（skill 生命周期）

agent 创建的 skill 的后台维护。跟踪使用情况，将闲置 skill 标记为过时，归档过时的 skill，保留运行前的 tar.gz 备份以防数据丢失。

- **CLI：** `atlaz curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
用户文档：https://atlaz.nousresearch.com/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `atlaz kanban <verb>` 驱动；调度器生成的 worker 看到由 `HERMES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
用户文档：https://atlaz.nousresearch.com/docs/user-guide/features/kanban
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`atlaz config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。
...
### 工具不可用
1. `atlaz tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `atlaz doctor` — 检查配置和依赖
2. `atlaz auth` — 重新认证 OAuth 提供商（或 `atlaz auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `atlaz model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `atlaz skills list` — 验证已安装
2. `atlaz skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `atlaz -s name`
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `atlaz config edit` 或[配置文档](https://atlaz.nousresearch.com/docs/user-guide/configuration) |
| 可用工具 | `atlaz tools list` 或[工具参考](https://atlaz.nousresearch.com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://atlaz.nousresearch.com/docs/reference/slash-commands) |
| Skill 目录 | `atlaz skills browse` 或[Skill 目录](https://atlaz.nousresearch.com/docs/reference/skills-catalog) |
| 提供商设置 | `atlaz model` 或[提供商指南](https://atlaz.nousresearch.com/docs/integrations/providers) |
| 平台设置 | `atlaz gateway setup` 或[消息文档](https://atlaz.nousresearch.com/docs/user-guide/messaging/) |
| MCP 服务器 | `atlaz mcp list` 或[MCP 指南](https://atlaz.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `atlaz profile list` 或[Profiles 文档](https://atlaz.nousresearch.com/docs/user-guide/profiles) |
| Cron 任务 | `atlaz cron list` 或[Cron 文档](https://atlaz.nousresearch.com/docs/user-guide/features/cron) |
| 记忆 | `atlaz memory status` 或[记忆文档](https://atlaz.nousresearch.com/docs/user-guide/features/memory) |
| 环境变量 | `atlaz config env-path` 或[环境变量参考](https://atlaz.nousresearch.com/docs/reference/environment-variables) |
| CLI 命令 | `atlaz --help` 或[CLI 参考](https://atlaz.nousresearch.com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.hermes/logs/gateway.log` |
| 会话文件 | `~/.hermes/sessions/` 或 `atlaz sessions browse` |
...
面向偶尔贡献者和 PR 作者。完整开发者文档：https://atlaz.nousresearch.com/docs/developer-guide/
...
├── atlaz_cli/           # CLI subcommands, config, setup, commands
...
1. 在 `atlaz_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- POSIX 文件模式（0o600 等）→ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`（参见 `tests/atlaz_cli/test_auth_toctou_file_modes.py`）
- `signal.SIGALRM` → 仅 Unix（参见 `tests/conftest.py::_enforce_test_timeout`）
