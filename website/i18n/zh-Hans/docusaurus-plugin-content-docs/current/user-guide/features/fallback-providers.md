...
ATLAZ 具备三层弹性机制，在提供商出现问题时保持会话正常运行：
...
`atlaz fallback` 复用 `atlaz model` 的提供商选择器——相同的提供商列表、相同的凭据提示、相同的验证流程。使用子命令 `add`、`list`（别名 `ls`）、`remove`（别名 `rm`）和 `clear` 来管理备用链。更改会持久化到 `config.yaml` 顶层的 `fallback_providers:` 列表中。
...
`fallback_model`（单数）是旧版单备用键——Hermes 仍支持以保持向后兼容。`fallback_providers`（复数，列表）支持按顺序尝试多个备用；`atlaz fallback` 写入此键。当两者同时设置时，Hermes 会合并它们，`fallback_providers` 优先。
:::
...
| Nous Portal | `nous` | `atlaz setup --portal`（全新安装）或 `atlaz auth add nous`（OAuth） |
...
| Google Gemini（OAuth） | `google-gemini-cli` | `atlaz model`（Google OAuth；可选：`HERMES_GEMINI_PROJECT_ID`） |
| Google AI Studio | `gemini` | `GOOGLE_API_KEY`（别名：`GEMINI_API_KEY`） |
| xAI（Grok） | `xai`（别名 `grok`） | `XAI_API_KEY`（可选：`XAI_BASE_URL`） |
| xAI Grok OAuth（SuperGrok） | `xai-oauth`（别名 `grok-oauth`） | `atlaz model` → xAI Grok OAuth（浏览器登录；需 SuperGrok 订阅） |
| AWS Bedrock | `bedrock` | 标准 boto3 认证（`AWS_REGION` + `AWS_PROFILE` 或 `AWS_ACCESS_KEY_ID`） |
| Qwen Portal（OAuth） | `qwen-oauth` | `atlaz model`（Qwen Portal OAuth；可选：`HERMES_QWEN_BASE_URL`） |
| MiniMax（OAuth） | `minimax-oauth` | `atlaz model`（MiniMax 门户 OAuth） |
...
| Triage Specifier | `atlaz kanban specify` / 看板（kanban）✨ 按钮——将单行 triage 任务扩展为完整规格 | `auxiliary.triage_specifier` |
...
| `"nous"` | 强制使用 Nous Portal | `atlaz auth` |
| `"codex"` | 强制使用 Codex OAuth | `atlaz model` → Codex |
