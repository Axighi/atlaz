...
如果你编写的是 worker 代码本身——即运行在通道*内部*的 agent——请参阅 [`kanban-worker`](https://github.com/Axighi/atlaz/blob/main/skills/devops/kanban-worker/SKILL.md) skill，其中包含更深入的操作细节。
...
对于 Hermes profile 通道，调度器的 `_default_spawn` 会在任务固定的工作区内运行 `atlaz -p <assignee> chat -q <prompt>`（或当 `hermes` shim 不在 `$PATH` 时使用等效的模块形式），并设置以下环境变量：
...
- **使用 block 而非 complete**，`reason` 以 `review-required: ` 为前缀，使仪表板 / `atlaz kanban show` 将该行显示为等待审查。
...
[`kanban-worker`](https://github.com/Axighi/atlaz/blob/main/skills/devops/kanban-worker/SKILL.md) skill 中有 `kanban_complete`（真正终态的任务——拼写修复、文档变更、研究报告）和 `review-required` block 模式的完整示例。
...
仪表板以摘要、元数据块和退出状态徽章渲染运行历史。CLI 用户可运行 `atlaz kanban tail <task_id>` 实时跟踪，或运行 `atlaz kanban runs <task_id>` 查看历史尝试列表。

## 现有通道形态

### Hermes profile 通道（默认）

当前所有 kanban worker 采用的形态：assignee 是 profile 名称，调度器生成 `atlaz -p <profile>`，worker 自动加载 [`kanban-worker`](https://github.com/Axighi/atlaz/blob/main/skills/devops/kanban-worker/SKILL.md) skill 以及 `KANBAN_GUIDANCE` 系统提示块，并使用 `kanban_*` 工具终止运行。除定义 profile 外无需任何额外配置。

为你的 fleet 创建 profile 时，选择与你希望 orchestrator 路由到的*角色*相匹配的名称。orchestrator（如果存在）通过 `atlaz profile list` 发现你的 profile 名称——系统不假设固定的名单（orchestrator 侧的契约请参阅 [`kanban-orchestrator`](https://github.com/Axighi/atlaz/blob/main/skills/devops/kanban-orchestrator/SKILL.md) skill）。
...
相关历史 issue 为 [#19931](https://github.com/Axighi/atlaz/issues/19931)，以及已关闭未合并的 Codex 专项 PR [#19924](https://github.com/Axighi/atlaz/pull/19924)——这些描述了原始架构提案，但未落地运行器。
...
- **滞留任务检测** — assignee 在 `kanban.stranded_threshold_seconds`（默认 30 分钟）内始终未产生 claim 的 ready 任务，会在 `atlaz kanban diagnostics` 中显示为 `stranded_in_ready` 警告。严重程度在 2 倍阈值时升级为 error，在 6 倍时升级为 critical。可通过单一信号捕获拼写错误的 assignee、已删除的 profile 以及宕机的外部 worker 池——与标识无关，无需维护每个看板的白名单。

## 相关资源

- [Kanban 概览](./kanban) — 面向用户的介绍。
- [Kanban 教程](./kanban-tutorial) — 开启仪表板的完整演练。
- [`kanban-worker`](https://github.com/Axighi/atlaz/blob/main/skills/devops/kanban-worker/SKILL.md) — worker 进程加载的 skill。
- [`kanban-orchestrator`](https://github.com/Axighi/atlaz/blob/main/skills/devops/kanban-orchestrator/SKILL.md) — orchestrator 侧。