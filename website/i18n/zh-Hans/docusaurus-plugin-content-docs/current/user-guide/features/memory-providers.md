...
ATLAZ 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。
...
也可以通过 `atlaz plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。
...
旧版 `atlaz honcho setup` 命令仍然有效（现在会重定向到 `atlaz memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。
...
:::tip 从 `atlaz honcho` 迁移
如果你之前使用过 `atlaz honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::
...
| `retain_context` | `conversation between ATLAZ and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/Axighi/atlaz/blob/main/plugins/memory/hindsight/README.md)。
