...
| 命令 | 功能 |
|---|---|
| `atlaz secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `atlaz secrets bitwarden status` | 显示配置、二进制版本及令牌是否存在 |
| `atlaz secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `atlaz secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `atlaz secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `atlaz secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |
...
| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `atlaz secrets bitwarden setup` |
