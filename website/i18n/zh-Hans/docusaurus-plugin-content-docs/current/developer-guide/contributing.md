...
sidebar_position: 4
title: "贡献指南"
description: "如何为 ATLAZ 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 ATLAZ 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- **Profile 安全路径**：不得硬编码 `~/.hermes` — 代码路径使用 `hermes_constants` 中的 `get_hermes_home()`，面向用户的消息使用 `display_hermes_home()`。完整规则参见 [AGENTS.md](https://github.com/Axighi/atlaz/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/Axighi/atlaz/issues)
- 请包含：操作系统、Python 版本、Hermes 版本（`atlaz version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/Axighi/atlaz/blob/main/LICENSE) 授权。