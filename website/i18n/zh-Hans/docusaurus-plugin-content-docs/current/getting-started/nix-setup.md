...
description: "使用 Nix 安装和部署 ATLAZ——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

ATLAZ 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`atlaz setup`、`atlaz gateway install`、编辑配置）与标准安装完全相同。
...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`atlaz setup` 引导你完成提供商选择，`atlaz gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
- 路由是透明的：`atlaz chat`、`atlaz sessions list`、`atlaz version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
| 被屏蔽的命令 | 原因 |
|---|---|
| `atlaz setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `atlaz config edit` | 配置由 `settings` 生成 |
| `atlaz config set <key> <value>` | 配置由 `settings` 生成 |
| `atlaz gateway install` | systemd 服务由 NixOS 管理 |
| `atlaz gateway uninstall` | systemd 服务由 NixOS 管理 |
...
NixOS 模块支持声明式插件安装——无需命令式的 `atlaz plugins install`。
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `hermes-agent` 二进制文件存在且 `atlaz version` 可运行 |
| `entry-points-sync` | `pyproject.toml` 中 `[project.scripts]` 的每个条目在 Nix 包中都有对应的封装二进制文件 |
| `cli-commands` | `atlaz --help` 暴露 `gateway` 和 `config` 子命令 |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `atlaz gateway` 的额外参数 |
...
| `atlaz version` 显示旧版本 | 容器未重启 | `systemctl restart hermes-agent` |
