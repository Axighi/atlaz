...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Hermes profile（配置文件）。覆盖 `atlaz profile use` 设置的粘性默认值。 |
...
| 命令 | 用途 |
|---------|---------|
| `atlaz chat` | 与 agent 进行交互式或单次聊天。 |
| `atlaz model` | 交互式选择默认 provider 和模型。 |
| `atlaz fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `atlaz gateway` | 运行或管理消息 gateway 服务。 |
| `atlaz proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `atlaz lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `atlaz setup` | 全部或部分配置的交互式设置向导。 |
| `atlaz whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `atlaz slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `atlaz auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `atlaz login` / `logout` | **已弃用** — 请改用 `atlaz auth`。 |
| `atlaz status` | 显示 agent、auth 和平台状态。 |
| `atlaz cron` | 检查并触发 cron 调度器。 |
| `atlaz kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `atlaz webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `atlaz hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `atlaz doctor` | 诊断配置和依赖问题。 |
| `atlaz security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `atlaz dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `atlaz debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `atlaz backup` | 将 Hermes 主目录备份为 zip 文件。 |
| `atlaz checkpoints` | 检查/修剪/清除 `~/.hermes/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `atlaz import` | 从 zip 文件恢复 Hermes 备份。 |
| `atlaz logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `atlaz config` | 显示、编辑、迁移和查询配置文件。 |
| `atlaz pairing` | 审批或撤销消息配对码。 |
| `atlaz skills` | 浏览、安装、发布、审计和配置 skill。 |
| `atlaz bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `atlaz curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `atlaz memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `atlaz honcho`）会自动注册。 |
| `atlaz acp` | 将 Hermes 作为 ACP 服务器运行，用于编辑器集成。 |
| `atlaz mcp` | 管理 MCP 服务器配置，并将 Hermes 作为 MCP 服务器运行。 |
| `atlaz plugins` | 管理 ATLAZ plugin（安装、启用、禁用、删除）。 |
| `atlaz portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
| `atlaz tools` | 按平台配置已启用的工具。 |
| `atlaz computer-use` | 安装或检查 cua-driver 后端（macOS Computer Use）。 |
| `atlaz sessions` | 浏览、导出、修剪、重命名和删除会话。 |
| `atlaz insights` | 显示 token/费用/活动分析。 |
| `atlaz claw` | OpenClaw 迁移辅助工具。 |
| `atlaz dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `atlaz profile` | 管理 profile——多个隔离的 Hermes 实例。 |
| `atlaz completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `atlaz version` | 显示版本信息。 |
| `atlaz update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `HERMES_HOME` 进行快照。 |
| `atlaz uninstall` | 从系统中删除 Hermes。 |
...
### `atlaz -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`atlaz -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `atlaz chat -q`；`-z` 专门用于"我只需要最终答案"的场景。
...
**`atlaz model`**（从终端运行，在任何 Hermes 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Hermes 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Hermes 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `atlaz model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `atlaz model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `HERMES_HOME`。当你并行运行多个 profile 并希望在 `atlaz update` 后全部重启时很有用。 |
...
:::tip WSL 用户
使用 `atlaz gateway run` 而非 `atlaz gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hermes 'hermes gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `atlaz setup` 现在默认执行此操作。 |
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `atlaz setup --portal`。
...
`atlaz update` 后重新运行 `atlaz slack manifest --write` 以获取新增命令。


## `atlaz login` / `atlaz logout` *（已弃用）*

:::caution
`atlaz login` 已被移除。请使用 `atlaz auth` 管理 OAuth 凭据，使用 `atlaz model` 选择 provider，或使用 `atlaz setup` 进行完整的交互式设置。
:::
...
| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `atlaz kanban boards switch`、`HERMES_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `atlaz kanban`。Worker 的环境中固定了 `HERMES_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
:::tip
`atlaz dump` 专为分享而设计。交互式诊断请使用 `atlaz doctor`。可视化概览请使用 `atlaz status`。
:::
...
| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `atlaz checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
Hermes 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`atlaz logs list` 子命令显示所有日志文件，包括已轮转的。
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `atlaz update` 后的第一次 tick 时立即执行 curator。使用 `atlaz curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `atlaz model` 相同的选择器）并追加到链末尾 |
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `atlaz <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `atlaz honcho`）。未激活的 provider 不暴露其子命令。运行 `atlaz --help` 查看当前已连接的命令。
:::
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `atlaz plugins` 会打开包含两个部分的复合交互界面：
...
`atlaz computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `atlaz tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`atlaz update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Hermes 更新时，使用此选项。
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.hermes/backups/pre-migration-*.zip`；可用 `atlaz import` 恢复）。 |
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过 PTY/WebSocket 桥接在后台运行 `atlaz --tui`，启用浏览器内 Chat 标签页。需要 `pip install 'hermes-agent[web,pty]'` 以及 Linux、macOS 或 WSL2 等 POSIX PTY 环境。 |
| `--insecure` | 关闭 | 允许绑定到非 localhost 主机。会在网络上暴露控制台凭据；仅在受信任的网络控制下使用。 |
| `--stop` | — | 停止正在运行的 `atlaz dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `atlaz dashboard` 进程并退出。 |
...
**pip 安装：** `atlaz update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade hermes-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`atlaz update` 也会在 `git pull` 前对 `~/.hermes/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `atlaz backup restore --state pre-update` 回滚。
...
| 命令 | 说明 |
|---------|-------------|
| `atlaz version` | 打印版本信息。 |
| `atlaz update` | 拉取最新变更并重新安装依赖。 |
| `atlaz uninstall [--full] [--yes]` | 删除 Hermes，可选择删除所有 config/数据。 |
