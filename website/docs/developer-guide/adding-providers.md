...
description: "How to add a new inference provider to ATLAZ — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---
...
- setup / `atlaz model` menu entries
...
1. `atlaz_cli/auth.py` decides how credentials are found.
2. `atlaz_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `atlaz_cli/models.py` and `atlaz_cli/main.py` make the provider show up in the CLI. (`atlaz_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `atlaz_cli/auth.py`
2. `atlaz_cli/models.py`
3. `atlaz_cli/runtime_provider.py`
4. `atlaz_cli/main.py`
...
:::tip
`atlaz_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `atlaz setup`.
:::
...
7. `atlaz model` menu includes the provider
8. `atlaz setup` wizard delegates to `main.py` automatically
...
- Provider-specific `atlaz model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `atlaz_cli/auth.py`
- `_PROVIDER_LABELS` in `atlaz_cli/models.py`
- `_PROVIDER_ALIASES` in both `atlaz_cli/auth.py` and `atlaz_cli/models.py`
- CLI `--provider` choices in `atlaz_cli/main.py`
...
## Step 2: Add auth metadata in `atlaz_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `atlaz_cli/models.py`
...
## Step 4: Resolve runtime data in `atlaz_cli/runtime_provider.py`
...
## Step 5: Wire the CLI in `atlaz_cli/main.py`

A provider is not discoverable until it shows up in the interactive `atlaz model` flow.

Update these in `atlaz_cli/main.py`:
...
:::tip
`atlaz_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `atlaz model` and `atlaz setup` automatically.
:::
...
- `tests/atlaz_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/atlaz_cli/test_model_switch_custom_providers.py` (and adjacent `tests/atlaz_cli/test_model_switch_*.py`)
- `tests/atlaz_cli/test_setup_model_provider.py`
...
python -m pytest tests/atlaz_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/atlaz_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m atlaz_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m atlaz_cli.main model
python -m atlaz_cli.main setup
```
...
- [ ] `ProviderConfig` added in `atlaz_cli/auth.py`
- [ ] aliases added in `atlaz_cli/auth.py` and `atlaz_cli/models.py`
- [ ] model catalog added in `atlaz_cli/models.py`
- [ ] runtime branch added in `atlaz_cli/runtime_provider.py`
- [ ] CLI wiring added in `atlaz_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `atlaz model` but not `atlaz setup`
