...
| `atlaz_cli/cron.py` | CLI `atlaz cron` subcommands |
...
In CLI mode, cron jobs only fire when `atlaz cron` commands are run or during active CLI sessions.
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `atlaz cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `atlaz cron` CLI provides direct job management:
