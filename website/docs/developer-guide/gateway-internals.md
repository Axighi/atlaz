...
1. `resolve_command()` from `atlaz_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`atlaz send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
- `atlaz gateway start` / `atlaz gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.hermes/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `atlaz gateway stop` stops only the current profile's gateway. `atlaz gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
