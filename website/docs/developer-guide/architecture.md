...
sidebar_position: 1
title: "Architecture"
description: "ATLAZ internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of ATLAZ internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│
├── atlaz_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `atlaz` subcommands (large file)
...
| **Profile isolation** | Each profile (`atlaz -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
