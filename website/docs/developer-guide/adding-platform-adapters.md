...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `atlaz config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
| `atlaz config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `atlaz status` | Shows plugin platforms with `(plugin)` tag |
| `atlaz gateway setup` | Plugin platforms appear in setup menu |
| `atlaz tools` / `atlaz skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.hermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `atlaz gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `atlaz cron run` separate from `atlaz gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `atlaz config`

`atlaz_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for ATLAZ.
author: Your Name
requires_env:
...
1. **`atlaz_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`atlaz_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`atlaz_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`atlaz_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`atlaz_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`atlaz_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
