...
This guide walks you through connecting ATLAZ to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- ATLAZ installed and running (`atlaz gateway`)
...
:::note
`atlaz webhook test <name>` only works for **dynamic subscriptions** created with `atlaz webhook subscribe`. It does not read routes from `config.yaml`.
:::
