...
If you're expecting jobs to fire automatically, you need a running gateway (`atlaz gateway` for foreground, or `atlaz gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `atlaz cron tick`.
...
If delivery fails, the job still runs — it just won't send anywhere. Check `atlaz cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `atlaz skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `atlaz skills list`.
...
3. The job's `last_run` metadata via `atlaz cron list`
...
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `atlaz skills install <skill-name>`.
...
1. Run the job with `atlaz cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.hermes/logs/agent.log` for scheduler messages and `~/.hermes/logs/errors.log` for warnings
3. Open an issue at [github.com/Axighi/atlaz](https://github.com/Axighi/atlaz) with:
