...
| | |
|---|---|
| Source | Optional — install with `atlaz skills install official/autonomous-ai-agents/honcho` |
...
Manual override: `atlaz honcho map my-project-name`
...
Run `atlaz honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `atlaz honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `atlaz profile create <name> --clone`. For existing profiles: `atlaz honcho sync`.
...
| Command | Description |
|---------|-------------|
| `atlaz honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `atlaz honcho status` | Show resolved config, connection test, peer info for active profile |
| `atlaz honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `atlaz honcho disable` | Disable Honcho for the active profile |
| `atlaz honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `atlaz honcho peers` | Show peer identities across all profiles |
| `atlaz honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `atlaz honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `atlaz honcho sessions` | List known directory-to-session-name mappings |
| `atlaz honcho map <name>` | Map current working directory to a Honcho session name |
| `atlaz honcho identity` | Seed AI peer identity or show both peer representations |
| `atlaz honcho sync` | Create host blocks for all Hermes profiles that don't have one yet |
| `atlaz honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hermes + Honcho |
| `atlaz memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `atlaz memory status` | Show active memory provider and config |
| `atlaz memory off` | Disable external memory provider |
