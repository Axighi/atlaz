...
<!-- ascii-guard-ignore -->
```
Python backend (atlaz_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `atlaz_cli/commands.py`. Autocomplete data ships from Python.
