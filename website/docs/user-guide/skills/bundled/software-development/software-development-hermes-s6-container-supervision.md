...
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← atlaz_cli.container_boot
...
| `docker/cont-init.d/02-reconcile-profiles` | Calls `atlaz_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
...
Edit `S6ServiceManager._render_run_script` in `atlaz_cli/service_manager.py`. The function is also called by `atlaz_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/atlaz_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
