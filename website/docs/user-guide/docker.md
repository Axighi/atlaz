...
sidebar_position: 7
title: "Docker"
description: "Running ATLAZ in Docker and using Docker as a terminal backend"
---

# ATLAZ — Docker

There are two distinct ways Docker intersects with ATLAZ:
...
If this is your first time running ATLAZ, create a data directory on the host and start the container interactively to run the setup wizard:
...
:::tip
Inside the container, run `atlaz setup --portal` once — the refresh token persists in the mounted `~/.hermes` volume. See [Nous Portal](/integrations/nous-portal).
:::
...
| `HERMES_DASHBOARD_TUI` | Set to `1` to expose the in-browser Chat tab (embedded `atlaz --tui` via PTY/WebSocket) | *(unset)* |
...
Each profile created with `atlaz profile create <name>` gets:
...
|---|---|---|
| **Per-profile gateway** (`atlaz gateway run` and per-profile gateways under s6) | Tee'd to two places: `docker logs <container>` (real time, no extra prefix) **and** `${HERMES_HOME}/logs/gateways/<profile>/current` (rotated, ISO-8601 timestamped, 10 archives × 1 MB each) | `docker logs -f hermes` or `tail -F ~/.hermes/logs/gateways/default/current` on the host |
...
4. Exec's the container's CMD as the main program (`/opt/hermes/docker/main-wrapper.sh`), which routes the arguments the user passed to `docker run`:
   - no args → `atlaz` (the default)
   - first arg is an executable on PATH (e.g. `sleep`, `bash`) → exec it directly
   - anything else → `atlaz <args>` (subcommand passthrough)
...
Do not override the image entrypoint unless you keep `/init` (or, equivalently, the legacy `docker/entrypoint.sh` shim that forwards to the stage2 hook) in the command chain. s6-overlay's `/init` runs as root so it can chown the volume on first boot, then drops to the `atlaz` user via `s6-setuidgid` for every supervised service AND for the main program. Starting `atlaz gateway run` as root inside the official image is refused by default because it can leave root-owned files in `/opt/data` and break later dashboard or gateway starts. Set `HERMES_ALLOW_ROOT_GATEWAY=1` only when you intentionally accept that risk.
:::

### `docker exec` automatically drops to the `atlaz` user
...
`atlaz status` inside the container reports `Manager: s6 (container supervisor)`. Use `/command/s6-svstat /run/service/gateway-<name>` for the raw supervisor view (note `/command/` is on PATH for supervision-tree processes only; pass the absolute path when calling from `docker exec`).
...
If a tool is likely to be useful to most ATLAZ users, consider contributing it upstream rather than carrying it in a private derived image. Open an issue or pull request on the [hermes-agent repository](https://github.com/Axighi/atlaz) describing the tool and its use case. Tools that get bundled into the official image benefit every user and avoid the maintenance overhead of a downstream fork.
...
The container's stage2 hook drops privileges to the non-root `atlaz` user (UID 10000) via `s6-setuidgid` inside each supervised service. If your host `~/.hermes/` is owned by a different UID, set `HERMES_UID`/`HERMES_GID` — or their `PUID`/`PGID` aliases, for parity with LinuxServer.io and NAS images — to match your host user, or ensure the data directory is writable:
...
`docker exec hermes <cmd>` automatically drops to UID 10000 too — see [`docker exec` automatically drops to the `atlaz` user](#docker-exec-automatically-drops-to-the-hermes-user) for details and the per-invocation opt-out.
