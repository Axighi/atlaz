...
…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `atlaz profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
- **You just want to back up a profile on your own machine.** Use [`atlaz profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — that's what those are for.
...
Recipients who run `atlaz profile update research-bot` will pull the latest.
...
`atlaz profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `atlaz profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
When the lead ships v1.1 (better SOUL, new skill), engineers run `atlaz profile update pr-reviewer` and everyone's on the new version within minutes.
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `atlaz profile info <name>` and hold off on updates until you're ready.
:::
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `atlaz -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `atlaz profile` — not a parallel command tree.
...
- Reserved profile names (`atlaz`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`atlaz profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — local backup / restore (not distribution)
