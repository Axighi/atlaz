...
> Run `atlaz gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
(Use `atlaz gateway start` only after `atlaz gateway install` has registered the systemd/launchd service.)
...
fixing, restart `atlaz gateway run`.
...
Check `atlaz gateway run` logs for the bound host/port. If the adapter bound to
