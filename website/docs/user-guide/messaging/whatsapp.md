...
sidebar_position: 5
title: "WhatsApp"
description: "Set up ATLAZ as a WhatsApp bot via the built-in Baileys bridge"
---
...
> Run `atlaz gateway setup` and pick **WhatsApp** for a guided walk-through.
...
3. Run `atlaz whatsapp` and scan the QR code from that WhatsApp account
...
- Agent responses are prefixed with "⚕ **ATLAZ**" by default. You can customize or disable this in `config.yaml`:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `atlaz whatsapp`. |
...
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `atlaz whatsapp` if needed. |
...
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `atlaz gateway install` to re-snapshot your current PATH into the plist, then `atlaz gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
