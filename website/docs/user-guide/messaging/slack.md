...
sidebar_position: 4
title: "Slack"
description: "Set up ATLAZ as a Slack bot using Socket Mode"
---

# Slack Setup

Connect ATLAZ to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
4. Enter an app name (e.g., "ATLAZ") and select your workspace
5. Click **Create App**
...
[`COMMAND_REGISTRY`](https://github.com/Axighi/atlaz/blob/main/atlaz_cli/commands.py)
...
When Hermes adds new commands (e.g. after `atlaz update`), regenerate
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@ATLAZ what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @ATLAZ`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @ATLAZ` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @ATLAZ` |
...
7. ✅ Bot was **invited** to the channel (`/invite @ATLAZ`)
