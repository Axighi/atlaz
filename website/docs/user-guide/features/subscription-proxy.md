...
interface in `atlaz_cli/proxy/adapters/`.
...
If you see `not logged in`, run `atlaz auth add nous`. If you see
...
`atlaz_cli/proxy/adapters/<provider>.py` and registering it in
