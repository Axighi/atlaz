...
description: "Plugins shipped with ATLAZ that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `atlaz memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `atlaz plugins list` and the interactive `atlaz plugins` UI), but none load until you explicitly enable them:
...
The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `atlaz plugins enable <name>`.
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `atlaz memory` and `atlaz plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Enabling:** `atlaz plugins enable disk-cleanup` (or check the box in `atlaz plugins`).

**Disabling again:** `atlaz plugins disable disk-cleanup`.
...
**Enabling:** `atlaz plugins enable security-guidance` (or check the box in `atlaz plugins`).

**Disabling again:** `atlaz plugins disable security-guidance`.
...
Or check the box in the interactive `atlaz plugins` UI. Then put the credentials in `~/.hermes/.env`:
...
Session grouping keys off the Hermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `atlaz chat` session lives under one Langfuse session.
...
**Disabling:** `atlaz plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
**Disabling:** `atlaz plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.hermes/cache/google_meet/` until you remove them.
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `atlaz dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.
...
- Manifest source is reported as `bundled` in `atlaz plugins list`
