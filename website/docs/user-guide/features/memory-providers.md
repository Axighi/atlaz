...
ATLAZ ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
You can also select the active memory provider via `atlaz plugins` → Provider Plugins → Memory Provider.
...
The legacy `atlaz honcho setup` command still works (it now redirects to `atlaz memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
:::tip Migrating from `atlaz honcho`
If you previously used `atlaz honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::
...
| `retain_context` | `conversation between ATLAZ and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Axighi/atlaz/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
