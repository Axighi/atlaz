...
from hermes_cli.config import get_atlaz_home, get_config_path, read_raw_config
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_atlaz_root()
    except Exception:
        return None
    profile_home = get_atlaz_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``get_default_atlaz_root()`` for a profile-shaped HERMES_HOME can
...
# ``get_default_atlaz_root()`` returns — ``~/.hermes`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`atlaz_constants.get_default_atlaz_root` returns — so
    ...
    from atlaz_constants import get_default_atlaz_root
    return get_default_atlaz_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from atlaz_constants import get_default_atlaz_root
        real_home_shared = (
            get_default_atlaz_root() / "shared" / NOUS_SHARED_STORE_FILENAME
