...
# display_atlaz_home is imported lazily at call sites to avoid ImportError
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    current_home = str(get_atlaz_home().resolve())
...
def _sync_atlaz_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HERMES_HOME``.

    Under ``sudo``, ``HERMES_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_atlaz_home` falls back to ``/root/.hermes`` — the wrong
...
from atlaz_constants import is_container, is_termux, is_wsl
...
def _profile_suffix() -> str:
    ...
    import hashlib
    import re
    from atlaz_constants import get_default_atlaz_root
    home = get_atlaz_home().resolve()
    default = get_default_atlaz_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(atlaz_home: str | None = None) -> str:
    ...
    Args:
        atlaz_home: Optional explicit HERMES_HOME path. Defaults to the current
            ``get_atlaz_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from atlaz_constants import get_default_atlaz_root
    home = Path(atlaz_home or str(get_atlaz_home())).resolve()
    default = get_default_atlaz_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def _atlaz_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HERMES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_atlaz_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_atlaz_home().resolve()
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    atlaz_home = get_atlaz_home()
    hermes_node = atlaz_home / "node" / "bin"
    if _is_dir(hermes_node):
        candidates.append(str(hermes_node))
    hermes_nm = atlaz_home / "node_modules" / ".bin"
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        atlaz_home = _atlaz_home_for_target_user(home_dir)
        profile_arg = _profile_arg(atlaz_home)
...
Environment="HERMES_HOME={atlaz_home}"
Restart=always
RestartSec=5
...
"""
    ...
    atlaz_home = str(get_atlaz_home().resolve())
    profile_arg = _profile_arg(atlaz_home)
...
def systemd_stop(system: bool = False):
    ...
    _sync_atlaz_home_from_systemd_unit(system=system)
    try:
...
def launchd_install(force: bool = False):
    ...
    from atlaz_constants import display_atlaz_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_atlaz_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from atlaz_constants import get_atlaz_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_atlaz_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_weixin():
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_atlaz_home())))
    except KeyboardInterrupt:
        print()
