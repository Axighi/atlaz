...
#   - cli.py, atlaz_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from atlaz_cli.config import get_config_path
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from atlaz_cli.config import load_config as _load
        cfg = _load() or {}
...
def handle_function_call(
    ...
    try:
        ...
        try:
            from atlaz_cli.plugins import invoke_hook
            invoke_hook(
                "post_tool_call",
