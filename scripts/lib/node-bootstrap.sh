...
#   2. ~/.atlaz/node/ from a prior Hermes-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.atlaz/node/ (always works, zero shell rc edits)
#
# Usage:
...
#   ATLAZ_HOME               (default: $HOME/.hermes)
# ============================================================================
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $ATLAZ_HOME/node/..."
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ATLAZ_HOME/node/"
    return 0
}
