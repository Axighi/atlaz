...
$RepoUrlHttps = "https://github.com/Axighi/atlaz.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Axighi/atlaz/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Axighi/atlaz/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Axighi/atlaz/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m atlaz_cli.main setup
    } else {
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Axighi/atlaz/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
