...
def resolve_skin() -> dict:
    try:
        from atlaz_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from atlaz_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from atlaz_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from atlaz_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from atlaz_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from atlaz_cli.banner import get_update_result
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from atlaz_cli.config import load_config as _load_full_cfg
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from atlaz_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: atlaz_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from atlaz_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from atlaz_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    ...
    ``atlaz_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m atlaz_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "atlaz_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from atlaz_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from atlaz_cli.config import is_managed, save_env_value
        from atlaz_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from atlaz_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from atlaz_cli.config import remove_env_value
