...
... [OUTPUT TRUNCATED - 641759 chars omitted out of 691759 total] ...
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ATLAZ_REDACT_SECRETS"] = str(redact).lower()
...
class HermesCLI:
    ...
    def run(self):
        ...
        try:
            _redact_raw = os.getenv("ATLAZ_REDACT_SECRETS") or os.getenv("HERMES_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(ATLAZ_REDACT_SECRETS={_redact_raw}). "
