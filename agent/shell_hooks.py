...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`atlaz_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ...
  ``ATLAZ_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``atlaz_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``atlaz_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from atlaz_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`atlaz_cli.plugins.get_pre_tool_call_block_message`.  This is
...
# ---------------------------------------------------------------------------
# Introspection (used by `atlaz hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`atlaz_cli.plugins.invoke_hook`
