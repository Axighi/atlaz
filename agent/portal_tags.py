...
The version is sourced live from ``atlaz_cli.__version__`` so it auto-aligns
...
``atlaz_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``atlaz_cli`` cannot be imported (should
