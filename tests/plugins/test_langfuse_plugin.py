...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_hermes_config(self, monkeypatch):
        ...
        # Drop any cached import of atlaz_cli.config.
        sys.modules.pop("atlaz_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "atlaz_cli.config" not in sys.modules, (
            "langfuse plugin imported atlaz_cli.config — regression toward "
