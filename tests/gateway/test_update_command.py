...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _atlaz_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._atlaz_home", tmp_path), \
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and atlaz_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m atlaz_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._atlaz_home", atlaz_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        # The update_cmd uses sys.executable -m atlaz_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "atlaz_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "atlaz_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        pending_path = atlaz_home / ".update_pending.json"
        assert pending_path.exists()
        data = json.loads(pending_path.read_text())
        assert data["platform"] == "telegram"
        assert data["chat_id"] == "99999"
        assert "timestamp" in data
        assert not (atlaz_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((atlaz_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (atlaz_home / ".update_pending.json").exists()
        assert not (atlaz_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (atlaz_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = atlaz_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (atlaz_home / ".update_output.txt").write_text("done")
        (atlaz_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (atlaz_home / ".update_pending.json").write_text(json.dumps(pending))
        (atlaz_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (atlaz_home / ".update_pending.json").write_text(json.dumps(pending))
        (atlaz_home / ".update_output.txt").write_text("x" * 5000)
        (atlaz_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (atlaz_home / ".update_pending.json").write_text(json.dumps(pending))
        (atlaz_home / ".update_output.txt").write_text("Traceback: boom")
        (atlaz_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = atlaz_home / ".update_pending.json"
        output_path = atlaz_home / ".update_output.txt"
        exit_code_path = atlaz_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
