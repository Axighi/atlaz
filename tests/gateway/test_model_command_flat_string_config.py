...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "atlaz_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_hermes_home() / config.yaml`` — point it here.
    monkeypatch.setattr("atlaz_constants.get_hermes_home", lambda: hermes_home)
    monkeypatch.setattr("atlaz_cli.config.get_hermes_home", lambda: hermes_home)
    return cfg_path
