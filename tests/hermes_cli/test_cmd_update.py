...
from atlaz_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        """
        from atlaz_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "atlaz_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "atlaz_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("atlaz_cli.config.check_config_version", return_value=(1, 2)), patch(
            "atlaz_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("atlaz_cli.main.sys") as mock_sys:
            ...
            from atlaz_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("atlaz_cli.profiles.list_profiles", return_value=all_profiles),
            patch("atlaz_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("atlaz_cli.profiles.list_profiles", return_value=[default_p]),
            patch("atlaz_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("atlaz_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("atlaz_cli.config.detect_install_method", return_value="pip")
    @patch("atlaz_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from atlaz_cli.main import _update_via_zip
