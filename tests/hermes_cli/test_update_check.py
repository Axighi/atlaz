"""Tests for the update check mechanism in atlaz_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("atlaz_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("atlaz_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import atlaz_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "atlaz_cli" / "banner.py"
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("atlaz_cli.banner.subprocess.run") as mock_run:
        with patch("atlaz_cli.banner.check_via_pypi", return_value=0):
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from atlaz_cli.main import _invalidate_update_cache
