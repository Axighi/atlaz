...
import atlaz_cli.gateway as gateway_cli
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_atlaz_home_for_target_user", lambda home: "/home/alice/.hermes")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestSystemUnitHermesHome:
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        atlaz_home = str(gateway_cli.get_atlaz_home().resolve())
        assert f'HERMES_HOME={atlaz_home}' in unit


class TestHermesHomeForTargetUser:
    """Unit tests for _atlaz_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("HERMES_HOME", raising=False)

        result = gateway_cli._atlaz_home_for_target_user("/home/alice")
...
class TestProfileArg:
    ...
    def test_default_hermes_home_returns_empty(self, tmp_path, monkeypatch):
        ...
        result = gateway_cli._profile_arg(str(atlaz_home))
        assert result == ""
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_atlaz_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gateway_cli, "get_atlaz_home", lambda: root_home / ".hermes")
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m atlaz_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m atlaz_cli.main gateway run
          - python path/to/atlaz_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m atlaz_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/atlaz_cli/main.py gateway run",
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import atlaz_cli.main as cli_main
        ...
        if project_root is None:
            import atlaz_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "atlaz_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "atlaz_cli.main", "gateway", "status", "-l", "--help"],
