"""Tests for the Phase 4 s6 dispatch helper in atlaz_cli.gateway.
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from atlaz_cli import gateway as gw
    monkeypatch.setattr(
        "atlaz_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "atlaz_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "atlaz_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "atlaz_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    from atlaz_cli import gateway as gw
    from atlaz_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "atlaz_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "atlaz_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "atlaz_cli.service_manager.detect_service_manager",
...
def _stub_execvp(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    ...
    monkeypatch.setattr("atlaz_cli.gateway.os.execvp", fake_execvp)
    return calls
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("atlaz_cli.gateway._profile_suffix", lambda: "")
