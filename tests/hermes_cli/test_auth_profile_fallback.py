...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from atlaz_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    atlaz_home = tmp_path / "classic"
    atlaz_home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))

    _write(atlaz_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    _write(atlaz_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from atlaz_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: HERMES_HOME is set to a custom path that is NOT under
    # ~/.hermes/profiles/ — get_default_atlaz_root() returns HERMES_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from atlaz_cli.auth import read_credential_pool, write_credential_pool
