...
def _setup_hermes_auth(
    ...
    *,
    ...
    auth_file = atlaz_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("atlaz_cli.auth.httpx.Client", _factory)
    return holder
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    atlaz_home.mkdir(parents=True, exist_ok=True)
    (atlaz_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    atlaz_home = tmp_path / "hermes"
    _setup_hermes_auth(atlaz_home, access_token="")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    atlaz_home = tmp_path / "hermes"
    _setup_hermes_auth(atlaz_home, refresh_token="")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    atlaz_home = tmp_path / "hermes"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_hermes_auth(atlaz_home, access_token=fresh)
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("atlaz_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def _seed_xai_oauth_state(
    atlaz_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    atlaz_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (atlaz_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    atlaz_home = tmp_path / "hermes"
    _seed_xai_oauth_state(atlaz_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
    ...
    monkeypatch.setattr("atlaz_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((atlaz_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    atlaz_home = tmp_path / "hermes"
    _seed_xai_oauth_state(atlaz_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
    ...
    monkeypatch.setattr("atlaz_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from atlaz_cli.auth import _xai_oauth_discovery
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("atlaz_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    atlaz_home = tmp_path / "hermes"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_hermes_auth(atlaz_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    ...
    from atlaz_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    # nothing to import.
    raw_after = json.loads((atlaz_home / "auth.json").read_text())
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(atlaz_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
    ...
    monkeypatch.setattr("atlaz_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    auth_path = atlaz_home / "auth.json"
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from atlaz_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("atlaz_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    atlaz_home = tmp_path / "hermes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hermes_auth(atlaz_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(atlaz_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
    ...
    (atlaz_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(atlaz_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(atlaz_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(atlaz_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    atlaz_home = tmp_path / "hermes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hermes_auth(atlaz_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
