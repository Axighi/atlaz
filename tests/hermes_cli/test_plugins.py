"""Tests for the Hermes plugin system (atlaz_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<atlaz_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<atlaz_home>/plugins/``; we derive
    ``<atlaz_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        ...
        if hermes_home_str:
            atlaz_home = Path(hermes_home_str)
        else:
            atlaz_home = base.parent
        atlaz_home.mkdir(parents=True, exist_ok=True)
        cfg_path = atlaz_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        atlaz_home = tmp_path / "hermes_test"
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="atlaz_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("atlaz_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        atlaz_home = tmp_path / "hermes_test"
        plugins_dir = atlaz_home / "plugins"
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("atlaz_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("atlaz_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("atlaz_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("atlaz_cli.plugins.PluginContext.dispatch_tool.__module__", "atlaz_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from atlaz_cli import plugins as plugins_mod
