...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<atlaz_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._atlaz_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_atlaz_home", tmp_path)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``atlaz_cli/main.py`` must set
