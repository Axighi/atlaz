...
``atlaz_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from atlaz_cli import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    atlaz_home.mkdir()
    (atlaz_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_atlaz_home() to re-resolve under the patched env.
    from importlib import reload

    import atlaz_cli.config as _hc_config
    reload(_hc_config)
...
def test_load_hermes_env_does_not_override_existing(tmp_path, monkeypatch):
    ...
    atlaz_home = tmp_path / ".hermes"
    atlaz_home.mkdir()
    (atlaz_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
