...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("atlaz_cli.setup.ensure_hermes_home", {}),
        ("atlaz_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("atlaz_cli.config.is_managed", {"return_value": False}),
        ("atlaz_cli.setup.load_config", {"return_value": {}}),
        ("atlaz_cli.setup.save_config", {}),
        ("atlaz_cli.setup.get_env_value", {"return_value": None}),
        ("atlaz_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("atlaz_cli.setup._print_setup_summary", {}),
        ("atlaz_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("atlaz_cli.setup.load_config", {"return_value": {}}),
        ("atlaz_cli.setup.save_config", {}),
        ("atlaz_cli.auth.get_active_provider", {"return_value": None}),
        ("atlaz_cli.setup.get_env_value", {"return_value": None}),
        ("atlaz_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="atlaz_cli.setup.prompt_choice",
                quick="atlaz_cli.setup._run_quick_setup",
                model="atlaz_cli.setup.setup_model_provider",
                terminal="atlaz_cli.setup.setup_terminal_backend",
                agent="atlaz_cli.setup.setup_agent_settings",
                gateway="atlaz_cli.setup.setup_gateway",
                tools="atlaz_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("atlaz_cli.setup.prompt_choice", {"return_value": 0}),
                first="atlaz_cli.setup._run_first_time_quick_setup",
            )
            from atlaz_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "atlaz_cli.setup.run_setup_wizard",
