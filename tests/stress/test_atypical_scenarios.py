    ...
    def wrap(fn):
        def run():
            ...
            for m in list(sys.modules.keys()):
                if m.startswith(("atlaz_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from atlaz_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from atlaz_cli.kanban_db import resolve_workspace
...
def _(home, kb):
    ...
    try:
        from atlaz_cli import web_server as ws  # noqa
    except Exception:
        pass
