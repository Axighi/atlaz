"""Tests for atlaz_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports atlaz_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported atlaz_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("atlaz_bootstrap", None)
    import atlaz_bootstrap  # noqa: WPS433
    return atlaz_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import atlaz_bootstrap as its
    ...
    # import atlaz_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "atlaz_cli/main.py",   # hermes CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import atlaz_bootstrap' and that
        ...
        ``atlaz_bootstrap`` as a top-level module) leaves hermes
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when atlaz_bootstrap hasn't been
        ...
        assert first_import_name == "atlaz_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'atlaz_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import atlaz_bootstrap' line to be the first import."
        )
