...
2. **Isolated HERMES_HOME.** HERMES_HOME points to a per-test tempdir so
   code reading ``~/.hermes/*`` via ``get_atlaz_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_atlaz_home()`` is a bug to fix at the callsite.)
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    #    ``~/.hermes/*`` via ``get_atlaz_home()`` now gets the tempdir.
    #
    ...
    #    ``Path.home() / ".hermes"`` instead of ``get_atlaz_home()``
    #    is a bug to fix at the callsite.
    fake_atlaz_home = tmp_path / "hermes_test"
    fake_atlaz_home.mkdir()
    (fake_atlaz_home / "sessions").mkdir()
    (fake_atlaz_home / "cron").mkdir()
    (fake_atlaz_home / "memories").mkdir()
    (fake_atlaz_home / "skills").mkdir()
    monkeypatch.setenv("HERMES_HOME", str(fake_atlaz_home))
    ...
    try:
        import atlaz_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_atlaz_home(_hermetic_environment):
...
# correlated with a ``tests/atlaz_cli/`` pytest run starting up.
#
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m atlaz_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
