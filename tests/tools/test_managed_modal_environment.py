...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "atlaz_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("atlaz_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "atlaz_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    atlaz_cli = types.ModuleType("atlaz_cli")
    atlaz_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["atlaz_cli"] = atlaz_cli
    sys.modules["atlaz_cli.config"] = types.SimpleNamespace(
