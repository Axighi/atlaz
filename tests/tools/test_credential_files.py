...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        atlaz_home.mkdir()
        (atlaz_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"HERMES_HOME": str(atlaz_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(atlaz_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        atlaz_home = tmp_path / ".hermes"
        atlaz_home.mkdir()
        (atlaz_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        atlaz_home = tmp_path / ".hermes"
        atlaz_home.mkdir()
        (atlaz_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        atlaz_home = tmp_path / ".hermes"
        atlaz_home.mkdir()
        (atlaz_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        atlaz_home = tmp_path / ".hermes"
        (atlaz_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above atlaz_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside atlaz_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        ...
        atlaz_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
        (atlaz_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside atlaz_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside atlaz_home pointing outside
        symlink = atlaz_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, atlaz_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(atlaz_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(atlaz_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (atlaz_home / "oauth.json").write_text("{}")
        self._write_config(atlaz_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        atlaz_home = tmp_path / ".hermes"
        atlaz_home.mkdir()
        (atlaz_home / "cache" / "documents").mkdir(parents=True)
        (atlaz_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(atlaz_home))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        atlaz_home = tmp_path / ".hermes"
        atlaz_home.mkdir()
        # Use legacy dir name — get_atlaz_dir prefers old if it exists
        (atlaz_home / "document_cache").mkdir()
        (atlaz_home / "image_cache").mkdir()
        monkeypatch.setenv("HERMES_HOME", str(atlaz_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(atlaz_home / "document_cache") in host_paths
        assert str(atlaz_home / "image_cache") in host_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        atlaz_home = tmp_path / ".hermes"
        doc_dir = atlaz_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        atlaz_home = tmp_path / ".hermes"
        ss_dir = atlaz_home / "cache" / "screenshots"
