...
from atlaz_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(atlaz_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <atlaz_home>/plugins/<name> and opt it in."""
    plugin_dir = atlaz_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    atlaz_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        atlaz_home, "capture_hook",
        register_body=(
