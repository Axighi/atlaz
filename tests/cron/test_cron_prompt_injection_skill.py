...
def cron_env(tmp_path, monkeypatch):
    ...
    skills_dir.mkdir()
    (atlaz_home / "cron").mkdir()
    (atlaz_home / "cron" / "output").mkdir()
    ...
    monkeypatch.setattr(_skills_tool, "HERMES_HOME", atlaz_home)
    ...
    return atlaz_home, _scheduler


def _plant_skill(atlaz_home: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.hermes/skills/<name>/ bypassing skills_guard."""
    skill_dir = atlaz_home / "skills" / name
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        atlaz_home, scheduler = cron_env
        _plant_skill(atlaz_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_invisible_unicode_raises(self, cron_env):
        ...
        _plant_skill(atlaz_home, "zwsp-skill", "clean looking\u200bskill content")
