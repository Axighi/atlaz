...
2. `atlaz_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `atlaz_cli/models.py` — Add Gemini model catalog
4. `atlaz_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `atlaz_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `atlaz_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- hermes-agent Copilot OAuth: `atlaz_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
